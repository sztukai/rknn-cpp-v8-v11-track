...
std::vector<STrack> BYTETracker::update(const std::vector<detect_result>& objects)
{
	...
	if (objects.size() > 0)
	{
		for (int i = 0; i < objects.size(); i++)
		{
			...
			std::vector<float> tlwh = strack._tlwh;
			int track_id = strack.track_id;
			// printf("frame_id: %d, ", this->frame_id);
			// printf("score: %f, track_thresh: %f\n", score, track_thresh);
			// printf("tlwh[0]: %.0f, tlwh[1]: %.0f, tlwh[2]: %.0f, tlwh[3]: %.0f\n", tlwh[0], tlwh[1], tlwh[2], tlwh[3]);
	...
	for (int i = 0; i < u_track.size(); i++)
	{
		...
		{
			std::vector<float> tlwh = track->_tlwh;
			int track_id = track->track_id;
			// printf("frame_id: %d, track_id: %d lost, ", this->frame_id, track_id);
			// printf("tlwh[0]: %.0f, tlwh[1]: %.0f, tlwh[2]: %.0f, tlwh[3]: %.0f\n", tlwh[0], tlwh[1], tlwh[2], tlwh[3]);
			track->mark_lost();
	...
	for (int i = 0; i < u_unconfirmed.size(); i++)
	{
		...
		std::vector<float> tlwh = track->_tlwh;
		int track_id = track->track_id;
		// printf("frame_id: %d, track_id: %d unconfirmed removed, ", this->frame_id, track_id);
		// printf("tlwh[0]: %.0f, tlwh[1]: %.0f, tlwh[2]: %.0f, tlwh[3]: %.0f\n", tlwh[0], tlwh[1], tlwh[2], tlwh[3]);
		track->mark_removed();
	...
	for (int i = 0; i < u_detection.size(); i++)
	{
		...
		std::vector<float> tlwh = track->_tlwh;
		int track_id = track->track_id;
		// printf("frame_id: %d, track_id: %d Appear, ", this->frame_id, track_id);
		// printf("tlwh[0]: %.0f, tlwh[1]: %.0f, tlwh[2]: %.0f, tlwh[3]: %.0f\n", tlwh[0], tlwh[1], tlwh[2], tlwh[3]);
		activated_stracks.push_back(*track);
	}
	...
	for (int i = 0; i < this->lost_stracks.size(); i++)
	{
		...
		{
			std::vector<float> tlwh = this->lost_stracks[i]._tlwh;
			int track_id = this->lost_stracks[i].track_id;
			// printf("frame_id: %d, track_id: %d lost_stracks removed, ", this->frame_id, track_id);
			// printf("tlwh[0]: %.0f, tlwh[1]: %.0f, tlwh[2]: %.0f, tlwh[3]: %.0f\n", tlwh[0], tlwh[1], tlwh[2], tlwh[3]);

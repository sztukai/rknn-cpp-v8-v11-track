...
float sigmoid(float x) { return 1 / (1 + fast_exp(-x)); }
                      ...
                      float threshold)
{
    ...
    // printf("score_thres_i8: %d, threshold: %f\n", score_thres_i8, threshold);
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        grid_h = app_ctx->output_attrs[score_idx].dims[2];
        grid_w = app_ctx->output_attrs[score_idx].dims[3];
